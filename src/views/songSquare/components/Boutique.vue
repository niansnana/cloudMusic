...
<template>
  <div class="vanImageToRadius">
    <van-grid :border="false" :column-num="3">
      <van-grid-item v-for="(item, index) in playBoutiqueData" :key="index" @click="goDetailFn(item)">
...
export default {
  ...
  methods: {
    getBoutiqueList () {
      this.$api.getPlayhigHqualityListFn().then(res => {
        this.playBoutiqueData = res.data.playlists
      })
    },
    goDetailFn (item) {
      this.$router.push({
